...
export async function getJSON(key){
  if(hasKV){ const v = await kv.get(key); if(!v) return null; if(typeof v === 'string'){ try{ return JSON.parse(v);}catch{ return v;} } return v; }
  const v = mem.get(key); return v ? JSON.parse(v) : null;
}
export async function setValue(key, val){ if(hasKV) return kv.set(key, val); mem.set(key, val); }
export async function getValue(key){ if(hasKV){ const v = await kv.get(key); return v ?? 0; } return mem.has(key) ? mem.get(key) : 0; }
