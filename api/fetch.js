...
async function parseFirst(feedUrl){ try{ const feed = await parser.parseURL(feedUrl); return feed.items?.[0] || null; }catch{ return null; } }
...
export default async function handler(req, res){
  try{
    ...
    for(const [cat, urls] of Object.entries(CATEGORY_SOURCES)){
      ...
      if(freshest){ const t = toTopic(freshest, cat); t.section = cat; topics.push(t); }
    }
    if(!topics.length){
      const fallbackFeeds = [...Object.values(MAIN_SOURCES).flat(), ...Object.values(CATEGORY_SOURCES).flat()];
    ...
    const seen = new Set();
    const unique = topics.filter(t => { const key=(t.title||'')+'|'+(t.source||''); if(seen.has(key)) return false; seen.add(key); return true; });
